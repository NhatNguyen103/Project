...
function Categories() {
    ...
    function filterData(products, selected, query) {
        ...
        if (selected) {
            filteredProducts = filteredProducts.filter(({ Category, color, newPrice, title, name, blaze }) =>
                Category === selected ||
                color === selected ||
                newPrice === selected ||
                name === selected ||
        ...
        return filteredProducts.map(({ pic, title, star, review, Price, newPrice, id, blaze }) => (
            <Card
                key={id}
                pic={pic}
                title={title}
